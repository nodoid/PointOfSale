...
Friend Class frmTopSelectNon
	...
	Private Sub loadLanguage()
		...
		'If rsLang.RecordCount Then frmTopSelect.Caption = rsLang("LanguageLayoutLnk_Description"): frmTopSelect.RightToLeft = rsLang("LanguageLayoutLnk_RightTL")
        ...
        'UPGRADE_ISSUE: Form property frmTopSelectNon.ToolTip1 was not upgraded. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="CC4C7EC0-C903-48FC-ACCC-81861D12DA4A"'
	...
	Private Sub cmdLoad_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles cmdLoad.Click
		...
		Select Case Me.cmbSortField.SelectedIndex
			...
			Case 5
				lOrder = "StockListNon.quantitySum"
        ...
        If Me._optType_0.Checked Then
            Report.Load("cryStockitemTop1Non.rpt")
        ElseIf Me._optType_1.Checked Then
            Report.Load("cryStockitemTopByGroupNon.rpt")
        Else
            Report.Load("cryStockitemTopGroupNon.rpt")
        End If
        Do While Report.DataDefinition.SortFields.Count
            'Report.DataDefinition.SortFields.delete(1)
        Loop
        ...
        If _optType_0.Checked Then
            If rs.Fields("Link_SQL").Value <> "" Then
                sql = "SELECT aStockItem1.StockItemID, aStockItem1.StockItem_Name, StockListNon.inclusiveSum AS inclusive, StockListNon.exclusiveSum AS exclusive, [exclusiveSum]-[depositSum] AS price, [exclusiveSum]-[depositSum] AS content, [exclusiveSum]-[depositSum]-[listCostSum] AS listProfit, [exclusiveSum]-[depositSum]-[actualCostSum] AS actualProfit, StockListNon.quantitySum AS quantity, StockListNon.listCostSum AS listCost, StockListNon.actualCostSum AS actualCost, IIf([exclusiveSum],([exclusiveSum]-[depositSum]-[actualCostSum])/[exclusiveSum]*100,0) AS gpActual, IIf([exclusiveSum],([exclusiveSum]-[depositSum]-[listCostSum])/[exclusiveSum]*100,0) AS gpList, aStockGroup.StockGroup_Name AS department FROM StockListNon INNER JOIN (aStockItem1 INNER JOIN aStockGroup ON aStockItem1.StockItem_StockGroupID = aStockGroup.StockGroupID) ON StockListNon.SaleItem_StockItemID = aStockItem1.StockItemID "
            Else
                Select Case Me.cmbSortField.SelectedIndex
                    Case 0
                        sql = "SELECT aStockItem1.StockItemID, aStockItem1.StockItem_Name, Sum(StockListNon.inclusiveSum) AS inclusive, Sum(StockListNon.exclusiveSum) AS exclusive, Sum([exclusiveSum]-[depositSum]) AS price, Sum([exclusiveSum]-[depositSum]) AS content, Sum([exclusiveSum]-[depositSum]-[listCostSum]) AS listProfit, Sum([exclusiveSum]-[depositSum]-[actualCostSum]) AS actualProfit, Sum(StockListNon.quantitySum) AS quantity, Sum(StockListNon.listCostSum) AS listCost, Sum(StockListNon.actualCostSum) AS actualCost, Sum(IIf([exclusiveSum],([exclusiveSum]-[depositSum]-[actualCostSum])/[exclusiveSum]*100,0)) AS gpActual, Sum(IIf([exclusiveSum],([exclusiveSum]-[depositSum]-[listCostSum])/[exclusiveSum]*100,0)) AS gpList, aStockGroup.StockGroup_Name AS department "
                        sql = sql & "FROM StockListNon INNER JOIN (aStockItem1 INNER JOIN aStockGroup ON aStockItem1.StockItem_StockGroupID = aStockGroup.StockGroupID) ON StockListNon.SaleItem_StockItemID = aStockItem1.StockItemID GROUP BY  aStockItem1.StockItemID, aStockItem1.StockItem_Name, aStockGroup.StockGroup_Name "
                    Case 1
                        sql = "SELECT aStockItem1.StockItemID, aStockItem1.StockItem_Name, Sum(StockListNon.inclusiveSum) AS inclusive, Sum(StockListNon.exclusiveSum) AS exclusive, Sum([exclusiveSum]-[depositSum]) AS price, Sum([exclusiveSum]-[depositSum]) AS content, Sum([exclusiveSum]-[depositSum]-[listCostSum]) AS listProfit, Sum([exclusiveSum]-[depositSum]-[actualCostSum]) AS actualProfit, Sum(StockListNon.quantitySum) AS quantity, Sum(StockListNon.listCostSum) AS listCost, Sum(StockListNon.actualCostSum) AS actualCost, Sum(IIf([exclusiveSum],([exclusiveSum]-[depositSum]-[actualCostSum])/[exclusiveSum]*100,0)) AS gpActual, Sum(IIf([exclusiveSum],([exclusiveSum]-[depositSum]-[listCostSum])/[exclusiveSum]*100,0)) AS gpList, aStockGroup.StockGroup_Name AS department "
                        sql = sql & "FROM StockListNon INNER JOIN (aStockItem1 INNER JOIN aStockGroup ON aStockItem1.StockItem_StockGroupID = aStockGroup.StockGroupID) ON StockListNon.SaleItem_StockItemID = aStockItem1.StockItemID GROUP BY  aStockItem1.StockItemID, aStockItem1.StockItem_Name, aStockGroup.StockGroup_Name, ([exclusiveSum]-[depositSum]-[listCostSum]) "
                        'lOrder = "[exclusiveSum]-[depositSum]-[listCostSum]"
                    Case 2
                        sql = "SELECT aStockItem1.StockItemID, aStockItem1.StockItem_Name, Sum(StockListNon.inclusiveSum) AS inclusive, Sum(StockListNon.exclusiveSum) AS exclusive, Sum([exclusiveSum]-[depositSum]) AS price, Sum([exclusiveSum]-[depositSum]) AS content, Sum([exclusiveSum]-[depositSum]-[listCostSum]) AS listProfit, Sum([exclusiveSum]-[depositSum]-[actualCostSum]) AS actualProfit, Sum(StockListNon.quantitySum) AS quantity, Sum(StockListNon.listCostSum) AS listCost, Sum(StockListNon.actualCostSum) AS actualCost, Sum(IIf([exclusiveSum],([exclusiveSum]-[depositSum]-[actualCostSum])/[exclusiveSum]*100,0)) AS gpActual, Sum(IIf([exclusiveSum],([exclusiveSum]-[depositSum]-[listCostSum])/[exclusiveSum]*100,0)) AS gpList, aStockGroup.StockGroup_Name AS department "
                    ...
                    Case 5
                        sql = "SELECT aStockItem1.StockItemID, aStockItem1.StockItem_Name, Sum(StockListNon.inclusiveSum) AS inclusive, Sum(StockListNon.exclusiveSum) AS exclusive, Sum([exclusiveSum]-[depositSum]) AS price, Sum([exclusiveSum]-[depositSum]) AS content, Sum([exclusiveSum]-[depositSum]-[listCostSum]) AS listProfit, Sum([exclusiveSum]-[depositSum]-[actualCostSum]) AS actualProfit, Sum(StockListNon.quantitySum) AS quantity, Sum(StockListNon.listCostSum) AS listCost, Sum(StockListNon.actualCostSum) AS actualCost, Sum(IIf([exclusiveSum],([exclusiveSum]-[depositSum]-[actualCostSum])/[exclusiveSum]*100,0)) AS gpActual, Sum(IIf([exclusiveSum],([exclusiveSum]-[depositSum]-[listCostSum])/[exclusiveSum]*100,0)) AS gpList, aStockGroup.StockGroup_Name AS department "
                        sql = sql & "FROM StockListNon INNER JOIN (aStockItem1 INNER JOIN aStockGroup ON aStockItem1.StockItem_StockGroupID = aStockGroup.StockGroupID) ON StockListNon.SaleItem_StockItemID = aStockItem1.StockItemID GROUP BY  aStockItem1.StockItemID, aStockItem1.StockItem_Name, aStockGroup.StockGroup_Name, StockListNon.quantitySum "
                        'lOrder = "StockListNon.quantitySum"
                End Select
        ...
        ElseIf _optType_1.Checked Then
            ...
            Select Case Me.cmbGroup.SelectedIndex
                Case 0
                    ...
                    Report.SetParameterValue("txtTitle", "Product Non-Performance - by Pricing Group")
                Case 1
                    Report.SetParameterValue("txtTitle", "Product Non-Performance - by Stock Group")
                Case 2
                    sql = Replace(sql, "StockGroup", "Supplier")
                    sql = Replace(sql, "aSupplier", "Supplier")
                    Report.SetParameterValue("txtTitle", "Product Non-Performance - by Supplier")
                Case 3
                    sql = "SELECT aStockItem1.StockItemID, aStockItem1.StockItem_Name, StockListNon.inclusiveSum AS inclusive, StockListNon.exclusiveSum AS exclusive, [exclusiveSum]-[depositSum] AS price, [exclusiveSum]-[depositSum] AS content, [exclusiveSum]-[depositSum]-[listCostSum] AS listProfit, [exclusiveSum]-[depositSum]-[actualCostSum] AS actualProfit, StockListNon.quantitySum AS quantity, StockListNon.listCostSum AS listCost, StockListNon.actualCostSum AS actualCost, IIf([exclusiveSum],([exclusiveSum]-[depositSum]-[actualCostSum])/[exclusiveSum]*100,0) AS gpActual, IIf([exclusiveSum],([exclusiveSum]-[depositSum]-[listCostSum])/[exclusiveSum]*100,0) AS gpList, aReportGroup1.ReportGroup_Name AS department FROM StockListNon INNER JOIN (aStockItem1 INNER JOIN aReportGroup1 ON aStockItem1.StockItem_ReportID = aReportGroup1.ReportID) ON StockListNon.SaleItem_StockItemID = aStockItem1.StockItemID "
                    Report.SetParameterValue("txtTitle", "Product Non-Performance - by Report Group")
            End Select
		...
		'for customer
		'SELECT aStockItem1.StockItemID, aStockItem1.StockItem_Name, StockListNon.inclusiveSum AS inclusive, StockListNon.exclusiveSum AS exclusive, [exclusiveSum]-[depositSum] AS price, [exclusiveSum]-[depositSum] AS content, [exclusiveSum]-[depositSum]-[listCostSum] AS listProfit, [exclusiveSum]-[depositSum]-[actualCostSum] AS actualProfit, StockListNon.quantitySum AS quantity, StockListNon.listCostSum AS listCost, StockListNon.actualCostSum AS actualCost, IIf([exclusiveSum],([exclusiveSum]-[depositSum]-[actualCostSum])/[exclusiveSum]*100,0) AS gpActual, IIf([exclusiveSum],([exclusiveSum]-[depositSum]-[listCostSum])/[exclusiveSum]*100,0) AS gpList, aCustomer.Customer_InvoiceName AS department
		'FROM aCustomer INNER JOIN (CustomerTransaction INNER JOIN (Sale INNER JOIN (SaleItem INNER JOIN (StockListNon INNER JOIN aStockItem1 ON StockListNon.SaleItem_StockItemID = aStockItem1.StockItemID) ON SaleItem.SaleItem_StockItemID = StockListNon.SaleItem_StockItemID) ON Sale.SaleID = SaleItem.SaleItem_SaleID) ON CustomerTransaction.CustomerTransaction_ReferenceID = Sale.SaleID) ON aCustomer.CustomerID = CustomerTransaction.CustomerTransaction_CustomerID
	...
	Private Sub cmdLoad_Click__OLD()
        ...
        If _optType_0.Checked Then
            sql = "SELECT aStockItem.StockItemID, aStockItem.StockItem_Name, StockListNon.inclusiveSum AS inclusive, StockListNon.exclusiveSum AS exclusive, [exclusiveSum]-[depositSum] AS price, [exclusiveSum]-[depositSum] AS content, [exclusiveSum]-[depositSum]-[listCostSum] AS listProfit, [exclusiveSum]-[depositSum]-[actualCostSum] AS actualProfit, StockListNon.quantitySum AS quantity, StockListNon.listCostSum AS listCost, StockListNon.actualCostSum AS actualCost, IIf([exclusiveSum],([exclusiveSum]-[depositSum]-[actualCostSum])/[exclusiveSum]*100,0) AS gpActual, IIf([exclusiveSum],([exclusiveSum]-[depositSum]-[listCostSum])/[exclusiveSum]*100,0) AS gpList, aStockGroup.StockGroup_Name AS department FROM StockListNon INNER JOIN (aStockItem INNER JOIN aStockGroup ON aStockItem.StockItem_StockGroupID = aStockGroup.StockGroupID) ON StockListNon.SaleItem_StockItemID = aStockItem.StockItemID "
        Else
            sql = "SELECT aStockItem.StockItemID, aStockItem.StockItem_Name, StockListNon.inclusiveSum AS inclusive, StockListNon.exclusiveSum AS exclusive, [exclusiveSum]-[depositSum] AS price, [exclusiveSum]-[depositSum] AS content, [exclusiveSum]-[depositSum]-[listCostSum] AS listProfit, [exclusiveSum]-[depositSum]-[actualCostSum] AS actualProfit, StockListNon.quantitySum AS quantity, StockListNon.listCostSum AS listCost, StockListNon.actualCostSum AS actualCost, IIf([exclusiveSum],([exclusiveSum]-[depositSum]-[actualCostSum])/[exclusiveSum]*100,0) AS gpActual, IIf([exclusiveSum],([exclusiveSum]-[depositSum]-[listCostSum])/[exclusiveSum]*100,0) AS gpList, aStockGroup.StockGroup_Name AS department FROM StockListNon INNER JOIN (aStockItem INNER JOIN aStockGroup ON aStockItem.StockItem_StockGroupID = aStockGroup.StockGroupID) ON StockListNon.SaleItem_StockItemID = aStockItem.StockItemID "
            Select Case Me.cmbGroup.SelectedIndex
                Case 0
    ...
    Private Sub frmTopSelectNon_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As KeyPressEventArgs) Handles MyBase.KeyPress
    ...
    Private Sub frmTopSelectNon_Leave(sender As Object, e As System.EventArgs) Handles Me.Leave

    End Sub
	Private Sub frmTopSelectNon_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub optType_CheckedChanged(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs)
        If eventSender.Checked Then
            ...
            Dim n As New RadioButton
            n = DirectCast(eventSender, RadioButton)
            Index = GetIndexer(n, optType)
            For Each m In optType
                If Index Then
