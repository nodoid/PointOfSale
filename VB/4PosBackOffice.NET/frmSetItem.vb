...
Friend Class frmSetItem
	...
	Public Sub loadItem(ByRef id As Integer)
		...
		gID = id
		rs = getRS("SELECT Set_Name FROM [Set] WHERE SetID = " & gID)
		...
		Else
			...
			rs = getRS("SELECT SetItem.SetItem_StockItemID From SetItem WHERE (((SetItem.SetItem_SetID)=" & gID & "));")
			Do Until rs.EOF
				For x = LBound(dataArray) To UBound(dataArray)
					If dataArray(x, 0) = rs.Fields("SetItem_StockItemID").Value Then
						dataArray(x, 2) = True
					End If
				Next x
				rs.moveNext()
	...
	Private Sub frmSetItem_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles MyBase.KeyPress
	...
	Private Sub lstStockItem_SelectedIndexChanged(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles lstStockItem.SelectedIndexChanged
		...
		If lstStockItem.SelectedIndex <> -1 Then
			...
			If dataArray(x, 2) Then
				...
				rs = getRS("SELECT SetItem.SetItem_StockItemID From SetItem WHERE (((SetItem.SetItem_SetID)<>" & gID & ") AND ((SetItem_StockItemID)=" & dataArray(x, 0) & "));")
				If rs.RecordCount Then
					MsgBox("This Stock Item already being used in other Stock Sets. Remove selected item from other stock sets first.", MsgBoxStyle.ApplicationModal + MsgBoxStyle.Information + MsgBoxStyle.OKOnly, My.Application.Info.Title)
					dataArray(x, 2) = 0
    ...
    Private Sub frmSetItem_Load(sender As Object, e As System.EventArgs) Handles Me.Load
