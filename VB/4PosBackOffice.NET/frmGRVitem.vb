...
Friend Class frmGRVitem
	...
	Private Sub loadLanguage()
		...
		'UPGRADE_ISSUE: Form property frmGRVitem.ToolTip1 was not upgraded. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="CC4C7EC0-C903-48FC-ACCC-81861D12DA4A"'
	...
	Private Sub getOrder()
		...
		With gridItem
			...
			Do Until rs.EOF
				...
				'End If
				If IsDbNull(rs.Fields("GRVItem_ContentCost").Value) Then
					MsgBox("This GRV has items on which is showing a zero cost. Please delete this GRV & recreate-it.")
				End If
				
				If CDec(rs.Fields("StockItem_ListCost").Value) <> CDec(IIf(IsDbNull(rs.Fields("GRVItem_ContentCost").Value), 0, rs.Fields("GRVItem_ContentCost").Value)) Then
					.Col = colContentExclusive
	...
	Private Sub getNamespace()
		...
		If rs.RecordCount Then
			Do Until rs.EOF
                lstWorkspace.Items.Add(New LBI(rs.Fields("name").Value & "", rs.Fields("StockItemID").Value))
				rs.moveNext()
			Loop 
		Else
	...
	Private Sub cmdBack_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles cmdBack.Click
		...
		If orderType = 1 Then
			orderType = 0
			loadItem(False)
			frmGRVitem_Resize(Me, New System.EventArgs())
		Else
			Me.Close()
		End If
    ...
    Private Sub doTotals()
        ...
        For x = 1 To gridItem.RowCount - 1
            ...
            lblDepositExclusive.Tag = CDbl(lblDepositExclusive.Tag) + CDec(gridItem.get_TextMatrix(x, colDepositTotalExclusive))
            lblDepositInclusive.Tag = CDbl(lblDepositInclusive.Tag) + CDec(gridItem.get_TextMatrix(x, colDepositTotalInclusive))
        ...
        lblDepositExclusive.Text = FormatNumber(lblDepositExclusive.Tag, 2)
        lblDepositInclusive.Text = FormatNumber(lblDepositInclusive.Tag, 2)
        lblTotalExclusive.Text = FormatNumber(CDec(lblContentExclusive.Tag) + CDec(lblDepositExclusive.Tag), 2)
        lblTotalInclusive.Text = FormatNumber(CDec(lblContentInclusive.Tag) + CDec(lblDepositInclusive.Tag), 2)
        ...
        Do Until rs.EOF
            ...
            Else
                Select Case LCase(rs.Fields("GRVTemplateList_Code").Value)
                    ...
                    Case "coldeposittotalexclusive" : gridItem.set_ColWidth(colDepositTotalExclusive, 0)
                    Case "coldeposittotalinclusive" : gridItem.set_ColWidth(colDepositTotalInclusive, 0)
    ...
    Private Sub frmGRVitem_KeyDown(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyDown
    ...
    Private Sub frmGRVitem_KeyPress(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.KeyPressEventArgs) Handles MyBase.KeyPress
    ...
    Private Sub frmGRVitem_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
    ...
    Private Sub frmGRVitem_Resize(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Resize
        Dim x As Short
        Dim lWidth As Short
        Dim lTop As Integer
        lTop = 120
    ...
    Private Sub frmGRVitem_FormClosed(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
        On Error Resume Next
        '*frmstocklist must not be displayed when you access frmstockitem from  frmGRVitem and exit stockitem,so that's what the code does
        Me.lblQuantity.Tag = -1
        Holdquantity = CInt(Me.lblQuantity.Tag)
        Me.lblQuantity.Tag = 0
        '*
    End Sub

    Private Sub gridItem_ClickEvent(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) 'Handles gridItem.ClickEvent
    ...
    End Sub
    Private Function findItem(ByRef id As Integer) As Object
        Dim x As Short
        With gridItem
    ...
    Private Sub txtEdit_TextChanged(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) 'Handles txtEdit.TextChanged
        ...
        Dim Index As Integer = GetIndexer(txtBox, txtEdit)
        If loading Then Exit Sub
        Dim lString As String
        ...
        Else
            ...
            If rj.RecordCount > 0 Then
                ...
                Else
                    ...
                    'update Cost price from GRV Deals
                    rsSql = getRS("SELECT GRVPromotion.PromotionID, GRVPromotionItem.PromotionItem_StockItemID, GRVPromotionItem.PromotionItem_Price, GRVPromotion.Promotion_StartDate, GRVPromotion.Promotion_EndDate FROM GRVPromotion INNER JOIN GRVPromotionItem ON GRVPromotion.PromotionID = GRVPromotionItem.PromotionItem_PromotionID WHERE (((GRVPromotionItem.PromotionItem_StockItemID)=" & rj.Fields("Catalogue_StockItemID").Value & ") AND ((GRVPromotion.Promotion_StartDate)<=#" & Format(Today, "yyyy/MM/dd") & "#) AND ((GRVPromotion.Promotion_EndDate)>=#" & Format(Today, "yyyy/MM/dd") & "#));")
                    If rsSql.RecordCount > 0 Then
                        If IsDBNull(rsSql.Fields("PromotionItem_Price").Value) Then
                        Else
                            sql = "UPDATE GRVItem SET GRVItem.GRVItem_ContentCost = " & rsSql.Fields("PromotionItem_Price").Value & " WHERE ((GRVItem.GRVItem_GRVID)=" & frmGRV.adoPrimaryRS.Fields("GRVID").Value & " AND GRVItem.GRVItem_Line =" & Me.gridItem.RowCount & ");"
...
gridItem_Err:
    ...
    Private Function moveNext(ByRef Index As Integer, ByRef direction As Integer) As Object
        Dim x As Short
        Dim y As Short
                ...
                If Chr(KeyAscii) = "." Then
                    ...
                    If txtBox.SelectionLength Then
                        lNewText = VB.Left(txtBox.Text, txtBox.SelectionStart) & _
                            Mid(txtBox.Text, txtBox.SelectionStart + txtBox.SelectionLength + 1)
                    End If
