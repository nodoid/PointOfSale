Option Strict Off
Option Explicit On
Friend Class frmFilterOrderList
    ...
    Public Function loadData(ByRef lName As String) As Boolean
        ...
        Loop
        rs.Close()
        rs = getRS("SELECT ftDataItem_ID From ftOrderItem WHERE (ftDataItem_PersonID = " & gPersonID & ") AND (ftDataItem_FieldName = '" & Replace(gField, "'", "''") & "')")
        Do Until rs.EOF
	...
	Private Sub save()
		...
		rs = cnnDB.Execute("DELETE FROM ftOrderItem WHERE (ftDataItem_PersonID = " & gPersonID & ") AND (ftDataItem_FieldName = '" & Replace(gField, "'", "''") & "')")
		cnnDB.Execute("DELETE FROM ftOrder WHERE (ftData_PersonID = " & gPersonID & ") AND (ftData_FieldName = '" & Replace(gField, "'", "''") & "')")
		For x = 0 To lstFilter.Items.Count - 1
			If lstFilter.GetItemChecked(x) Then
                ...
                cnnDB.Execute("INSERT INTO ftOrderItem (ftDataItem_PersonID, ftDataItem_FieldName, ftDataItem_ID) VALUES (" & gPersonID & ", '" & Replace(gField, "'", "''") & "', " & gArray(GetItemData(lstFilter, x), 0) & ")")
			End If
		...
		If lSQL <> "" Then
			...
			cnnDB.Execute("INSERT INTO ftOrder (ftData_PersonID, ftData_FieldName, ftData_SQL, ftData_Heading) VALUES (" & gPersonID & ", '" & Replace(gField, "'", "''") & "', '" & Replace(lSQL, "'", "''") & "', '" & Replace(lHeading, "'", "''") & "')")
		End If
	End Sub
	...
	Private Sub frmFilterOrderList_FormClosed(ByVal eventSender As System.Object, ByVal eventArgs As System.Windows.Forms.FormClosedEventArgs) Handles Me.FormClosed
		save()
	End Sub
	...
	Private Sub tbStockItem_ButtonClick(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles _tbStockItem_Button1.Click, _tbStockItem_Button2.Click, _tbStockItem_Button3.Click, _tbStockItem_Button4.Click
		...
		Else
			For x = 1 To tbStockItem.Items.Count
    ...
    Private Sub frmFilterOrderList_Load(sender As Object, e As System.EventArgs) Handles Me.Load
