...
Hell: 
	...
	Private Function getMasterDB() As Object
...
openConnection_Error: 
	...
	End Sub
    Public Function getRSwaitron(ByRef sql As String) As ADODB.Recordset
	...
	End Sub
	Private Sub frmSelCompChk_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
	...
	'        End If
	'End Sub
	Public Function loadDB(ByRef arrComp() As String) As Boolean
        Dim loadData As Boolean
        Dim loading As Boolean
        Dim gHeading As String
        Dim gFieldName As String
        Dim gFieldID As Integer
	...
	Private Sub lvLocation_DoubleClick(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles lvLocation.DoubleClick
		...
		Else
            loadDBStr = CBool(Me.lvLocation.FocusedItem.SubItems(2).Text)
			'Exit Sub
			Me.Close()
			Exit Sub
		End If
	...
	Private Sub loadCompanies()
        Dim locationCompany_Name As String
        Dim locationCompanyID As Integer
        Dim x As Integer
		...
		If rs.RecordCount Then
			x = -1
			Dim gArray(rs.RecordCount - 1, 2) As Object
			Do Until rs.EOF
				x = x + 1
