...
{
	internal partial class frmVATList : System.Windows.Forms.Form
	{
		string gFilter;
		ADODB.Recordset gRS;
		string gFilterSQL;
		...
		{
			My.MyProject.Forms.frmVAT.loadItem(ref 0);
			doSearch();
		}
		...
		private void DataList1_DblClick(System.Object eventSender, KeyPressEventArgs eventArgs)
		{
			if (!string.IsNullOrEmpty(DataList1.BoundText)) {
				My.MyProject.Forms.frmVAT.loadItem(ref Convert.ToInt32(DataList1.BoundText));
			}
			doSearch();
		}
		...
		private void frmVATList_KeyPress(System.Object eventSender, System.Windows.Forms.KeyPressEventArgs eventArgs)
		{
		...
		private void frmVATList_FormClosed(System.Object eventSender, System.Windows.Forms.FormClosedEventArgs eventArgs)
		{
			gRS.Close();
		}
		...
		private void doSearch()
		{
			...
			if (string.IsNullOrEmpty(lString)) {
				...
				lString = "WHERE (VAT_Name LIKE '%" + Strings.Replace(lString, " ", "%' AND VAT_Name LIKE '%") + "%')";
			}
			gRS = modRecordSet.getRS(ref "SELECT DISTINCT VATID, VAT_Name FROM VAT " + lString + " ORDER BY VAT_Name");
