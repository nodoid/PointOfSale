...
{
	internal partial class frmGRVPromotionList : System.Windows.Forms.Form
	{
		string gFilter;
		ADODB.Recordset gRS;
		string gFilterSQL;
		...
		private void loadLanguage()
		{
			...
			modRecordSet.rsLang.filter = "LanguageLayoutLnk_LanguageID=" + 1133;
			//Select a Promotion|Checked
			//If rsLang.RecordCount Then frmPromotionList.Caption = rsLang("LanguageLayoutLnk_Description"): frmPromotionList.RightToLeft = rsLang("LanguageLayoutLnk_RightTL")
			...
			//UPGRADE_ISSUE: Form property frmGRVPromotionList.ToolTip1 was not upgraded. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="CC4C7EC0-C903-48FC-ACCC-81861D12DA4A"'
		...
		{
			My.MyProject.Forms.frmGRVPromotion.loadItem(ref 0);
			doSearch();
		}
		...
		private void DataList1_DblClick(System.Object eventSender, System.EventArgs eventArgs)
		{
			if (!string.IsNullOrEmpty(DataList1.BoundText)) {
				My.MyProject.Forms.frmGRVPromotion.loadItem(ref Convert.ToInt32(DataList1.BoundText));
			}
			doSearch();
		}
		...
		private void frmGRVPromotionList_KeyPress(System.Object eventSender, System.Windows.Forms.KeyPressEventArgs eventArgs)
		{
		...
		private void frmGRVPromotionList_FormClosed(System.Object eventSender, System.Windows.Forms.FormClosedEventArgs eventArgs)
		{
			gRS.Close();
		}
		...
		private void doSearch()
		{
			...
			if (string.IsNullOrEmpty(lString)) {
				...
				lString = "WHERE (Promotion_Name LIKE '%" + Strings.Replace(lString, " ", "%' AND Promotion_Name LIKE '%") + "%')";
			}
			gRS = modRecordSet.getRS(ref "SELECT DISTINCT PromotionID, Promotion_Name FROM GRVPromotion " + lString + " ORDER BY Promotion_Name");
