...
{
	internal partial class frmPricingGroupList : System.Windows.Forms.Form
	{
		string gFilter;
		ADODB.Recordset gRS;
		string gFilterSQL;
		...
		private void loadLanguage()
		{
			...
			//frmPricingGroupList = No Code
			//rsLang.filter = "LanguageLayoutLnk_LanguageID=" & 0000
			//If rsLang.RecordCount Then frmPricingGroupList.Caption = rsLang("LanguageLayoutLnk_Description"): frmPricingGroupList.RightToLeft = rsLang("LanguageLayoutLnk_RightTL")
			...
			//UPGRADE_ISSUE: Form property frmPricingGroupList.ToolTip1 was not upgraded. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="CC4C7EC0-C903-48FC-ACCC-81861D12DA4A"'
		...
		{
			My.MyProject.Forms.frmPricingGroup.loadItem(ref 0);
			doSearch();
		}
		...
		private void DataList1_DblClick(System.Object eventSender, KeyPressEventArgs eventArgs)
		{
			if (cmdNew.Visible) {
				if (!string.IsNullOrEmpty(DataList1.BoundText)) {
					My.MyProject.Forms.frmPricingGroup.loadItem(ref Convert.ToInt32(DataList1.BoundText));
				}
				doSearch();
			} else {
				if (string.IsNullOrEmpty(DataList1.BoundText)) {
		...
		private void frmPricingGroupList_Load(System.Object eventSender, System.EventArgs eventArgs)
		{
			doSearch();
		}

		private void frmPricingGroupList_FormClosed(System.Object eventSender, System.Windows.Forms.FormClosedEventArgs eventArgs)
		{
			gRS.Close();
		}
		...
		private void doSearch()
		{
			...
			if (string.IsNullOrEmpty(lString)) {
				lString = "WHERE PricingGroup_Disabled = 0";
			} else {
				lString = "WHERE (PricingGroup_Name LIKE '%" + Strings.Replace(lString, " ", "%' AND PricingGroup_Name LIKE '%") + "%') AND (PricingGroup_Disabled = 0)";
			}
			gRS = modRecordSet.getRS(ref "SELECT DISTINCT PricingGroupID, PricingGroup_Name FROM PricingGroup " + lString + " ORDER BY PricingGroup_Name");
			//Display the list of Titles in the DataCombo
			DataList1.DataSource = gRS;
			DataList1.listField = "PricingGroup_Name";
			...
			DataList1.DataSource = gRS;
			DataList1.boundColumn = "PricingGroupID";
