...
{
	internal partial class frmGRVitem : System.Windows.Forms.Form
	{
//Option Explicit
...
//Public gNodeTemplate As IXMLDOMNode
		...
		private void getOrder()
		{
			...
			while (!(rs.EOF)) {
				...
				//End If
				if (Information.IsDBNull(rs.Fields("GRVItem_ContentCost").Value)) {
					Interaction.MsgBox("This GRV has items on which is showing a zero cost. Please delete this GRV & recreate-it.");
				}

				if (Convert.ToDecimal(rs.Fields("StockItem_ListCost").Value) != Convert.ToDecimal((Information.IsDBNull(rs.Fields("GRVItem_ContentCost").Value) ? 0 : rs.Fields("GRVItem_ContentCost").Value))) {
					_with1.Col = colContentExclusive;
		...
		private void cmdBack_Click(System.Object eventSender, System.EventArgs eventArgs)
		{
			...
			if (orderType == 1) {
				orderType = 0;
				loadItem(ref false);
				frmGRVitem_Resize(this, new System.EventArgs());
			} else {
				this.Close();
			}
		...
		private void doTotals()
		{
			...
			for (x = 1; x <= gridItem.RowCount - 1; x++) {
				...
				lblDepositExclusive.Tag = Convert.ToDouble(lblDepositExclusive.Tag) + Convert.ToDecimal(gridItem.get_TextMatrix(ref x, ref colDepositTotalExclusive));
				lblDepositInclusive.Tag = Convert.ToDouble(lblDepositInclusive.Tag) + Convert.ToDecimal(gridItem.get_TextMatrix(ref x, ref colDepositTotalInclusive));
			...
			lblDepositExclusive.Text = Strings.FormatNumber(lblDepositExclusive.Tag, 2);
			lblDepositInclusive.Text = Strings.FormatNumber(lblDepositInclusive.Tag, 2);
			lblTotalExclusive.Text = Strings.FormatNumber(Convert.ToDecimal(lblContentExclusive.Tag) + Convert.ToDecimal(lblDepositExclusive.Tag), 2);
			lblTotalInclusive.Text = Strings.FormatNumber(Convert.ToDecimal(lblContentInclusive.Tag) + Convert.ToDecimal(lblDepositInclusive.Tag), 2);
				...
				if (rs.Fields("visible").Value) {
					...
					switch (Strings.LCase(rs.Fields("GRVTemplateList_Code").Value)) {
						...
						case "coldepositinclusive":
							...
							break;
						case "coldeposittotalexclusive":
							gridItem.set_ColWidth(ref colDepositTotalExclusive, ref 0);
							break;
						case "coldeposittotalinclusive":
							gridItem.set_ColWidth(ref colDepositTotalInclusive, ref 0);
							break;
						case "coldiscountline":
		...
		private void frmGRVitem_KeyDown(System.Object eventSender, System.Windows.Forms.KeyEventArgs eventArgs)
		{
		...
		private void frmGRVitem_KeyPress(System.Object eventSender, System.Windows.Forms.KeyPressEventArgs eventArgs)
		{
		...
		private void frmGRVitem_Load(System.Object eventSender, System.EventArgs eventArgs)
		{
		...
		private void frmGRVitem_Resize(System.Object eventSender, System.EventArgs eventArgs)
		{
			short x = 0;
			short lWidth = 0;
			int lTop = 0;
		...
		private void frmGRVitem_FormClosed(System.Object eventSender, System.Windows.Forms.FormClosedEventArgs eventArgs)
		{
			 // ERROR: Not supported in C#: OnErrorStatement

			//*frmstocklist must not be displayed when you access frmstockitem from  frmGRVitem and exit stockitem,so that's what the code does
			this.lblQuantity.Tag = -1;
			modApplication.Holdquantity = Convert.ToInt32(this.lblQuantity.Tag);
			this.lblQuantity.Tag = 0;
			//*
		}

		//Handles gridItem.ClickEvent
		...
		private object findItem(ref int id)
		{
		...
		private void txtEdit_TextChanged(System.Object eventSender, System.EventArgs eventArgs)
		{
			...
			int Index = GetIndex.GetIndexer(ref txtBox, ref txtEdit);
			if (loading)
				return;
			string lString = null;
			int lValue = 0;
			...
			if (string.IsNullOrEmpty(Strings.Trim(lString))) {
				...
				if (rj.RecordCount > 0) {
					...
					if (x) {
						...
						//update Cost price from GRV Deals
						rsSql = modRecordSet.getRS(ref "SELECT GRVPromotion.PromotionID, GRVPromotionItem.PromotionItem_StockItemID, GRVPromotionItem.PromotionItem_Price, GRVPromotion.Promotion_StartDate, GRVPromotion.Promotion_EndDate FROM GRVPromotion INNER JOIN GRVPromotionItem ON GRVPromotion.PromotionID = GRVPromotionItem.PromotionItem_PromotionID WHERE (((GRVPromotionItem.PromotionItem_StockItemID)=" + rj.Fields("Catalogue_StockItemID").Value + ") AND ((GRVPromotion.Promotion_StartDate)<=#" + Strings.Format(DateAndTime.Today, "yyyy/MM/dd") + "#) AND ((GRVPromotion.Promotion_EndDate)>=#" + Strings.Format(DateAndTime.Today, "yyyy/MM/dd") + "#));");
						if (rsSql.RecordCount > 0) {
							if (Information.IsDBNull(rsSql.Fields("PromotionItem_Price").Value)) {
							} else {
								sql = "UPDATE GRVItem SET GRVItem.GRVItem_ContentCost = " + rsSql.Fields("PromotionItem_Price").Value + " WHERE ((GRVItem.GRVItem_GRVID)=" + My.MyProject.Forms.frmGRV.adoPrimaryRS.Fields("GRVID").Value + " AND GRVItem.GRVItem_Line =" + this.gridItem.RowCount + ");";
		...
		private object moveNext(ref int Index, ref int direction)
		{
			short x = 0;
			short y = 0;
