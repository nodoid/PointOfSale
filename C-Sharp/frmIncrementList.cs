...
{
	internal partial class frmIncrementList : System.Windows.Forms.Form
	{
		string gFilter;
		ADODB.Recordset gRS;
		string gFilterSQL;
		...
		private void loadLanguage()
		{
			...
			//NOTE: DB Entry 1158 closest match to caption, but better grammar (no "..." in field)
			modRecordSet.rsLang.filter = "LanguageLayoutLnk_LanguageID=" + 1158;
			//Select Price Increment Group
			...
			//UPGRADE_ISSUE: Form property frmIncrementList.ToolTip1 was not upgraded. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="CC4C7EC0-C903-48FC-ACCC-81861D12DA4A"'
		...
		{
			My.MyProject.Forms.frmIncrement.loadItem(ref 0);
			doSearch();
		}
		...
		private void DataList1_DblClick(System.Object eventSender, KeyEventArgs eventArgs)
		{
			if (cmdNew.Visible) {
				if (!string.IsNullOrEmpty(DataList1.BoundText)) {
					My.MyProject.Forms.frmIncrement.loadItem(ref Convert.ToInt32(DataList1.BoundText));
				}
				doSearch();
			} else {
				if (string.IsNullOrEmpty(DataList1.BoundText)) {
		...
		private void frmIncrementList_KeyPress(System.Object eventSender, System.Windows.Forms.KeyPressEventArgs eventArgs)
		{
		...
		private void frmIncrementList_FormClosed(System.Object eventSender, System.Windows.Forms.FormClosedEventArgs eventArgs)
		{
			gRS.Close();
		}
		...
		private void doSearch()
		{
			...
			if (string.IsNullOrEmpty(lString)) {
				...
				lString = "WHERE (Increment_Name LIKE '%" + Strings.Replace(lString, " ", "%' AND Increment_Name LIKE '%") + "%')";
			}
			gRS = modRecordSet.getRS(ref "SELECT DISTINCT IncrementID, Increment_Name FROM Increment " + lString + " ORDER BY Increment_Name");
			//Display the list of Titles in the DataCombo
			DataList1.DataSource = gRS;
			DataList1.listField = "Increment_Name";
			...
			DataList1.DataSource = gRS;
			DataList1.boundColumn = "IncrementID";
